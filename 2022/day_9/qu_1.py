with open("input.txt") as file:
    lines = file.read().splitlines()
    
    def calculate_tail_pos(h_pos, t_pos, direction):
        tail_will_move = False
        new_pos = t_pos[:]
        for i in range(2):
            if abs(h_pos[i]-t_pos[i]) > 1:
                tail_will_move = True
        if tail_will_move:
            new_pos = [int((h_pos[i]+t_pos[i]) / 2) for i in range(2)]
            if "L" in direction or "R" in direction:
                new_pos[1] = h_pos[1]
            else:
                new_pos[0] = h_pos[0]
        
        return new_pos
        
    
    h_pos = [0,0]
    t_pos = [0,0]
    visited_spots = {str(t_pos)}
    
    for line in lines:
        direction, distance = line.split(" ")
        distance = int(distance)
        
        while distance > 0:
            if "U" in direction: h_pos[1] += 1
            elif "D" in direction: h_pos[1] -= 1
            elif "L" in direction: h_pos[0] -= 1
            elif "R" in direction: h_pos[0] += 1

            t_pos = calculate_tail_pos(h_pos, t_pos, direction)
            distance -= 1
            visited_spots.add(str(t_pos))
    print(visited_spots)
    print(len(visited_spots))
            