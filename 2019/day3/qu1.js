...
const solver = (A, B) => {
  ...
  for (j in B) {
    ...
    if (direction === 'U') {
      for (i = current.y + 1; i <= current.y + distance; i++) {
        if (passedPoints[`${current.x}_${i}`] && closestCrossing > Math.abs(i) + Math.abs(current.x)) {
      ...
      for (i = current.x + 1; i <= current.x + distance; i++) {
        if (passedPoints[`${i}_${current.y}`] && closestCrossing > Math.abs(i) + Math.abs(current.y)) {
